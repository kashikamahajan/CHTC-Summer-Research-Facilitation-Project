...
# function to make sure the valeus from csv are 
def safe_float(val):
    try:
        return float(val)
...
def extract_requested_vs_used(jobs, selected_params):
    ...
    for job in jobs:
        ...
        for param in selected_params:
            val = safe_float(job.get(param))
            
            raw_val = job.get(param)
            val = safe_float(raw_val) # safe conversion to float
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python print_resource_usage_table.py <ClusterId> [param1 param2 ...]")
        sys.exit(1)
